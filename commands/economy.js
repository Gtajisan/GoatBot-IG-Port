...
module.exports = {
  ...
  async onStart({ api, event, args, commandName, database, usersData }) {
    const userId = event.senderID;
    const sub = ['bal', 'balance', 'daily', 'pay', 'wallet'].includes(commandName) ? commandName : (args[0] || 'balance').toLowerCase();

    if (['balance', 'bal', 'wallet'].includes(sub)) {
    ...
    if (sub === 'daily') {
      ...
      database.updateUser(userId, { money: eco.balance });
      database.save();
